...
if __name__ == "__main__":
    ...
    web = requests.get("https://anigod.com/animations/finale", headers=header)
    text = web.text
    patt = r'<a class="table-link" href="(\/animation\/[\s\S]*?)"[\s\S]*?>[\s\S]*?<\/a>'
    ...
    with db_adder.Timer() as t, ThreadPoolExecutor(max_workers=20) as executor:
        ...
        db.anime2.insert(result)

    for ani in db.anime2.find():
        print(ani)
